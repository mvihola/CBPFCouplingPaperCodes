import SimpleSMC: particleCopy!, _copyParticles!, _mutateParticles!,
  _intermediateOutput!, pickParticle!, _logWeightParticles!, 
...
function _pickParticlesBS!(ccsmcio::CCSMCIO{Particle}, lM::F, lG::FG) where
  {Particle, F<:Function, FG<:Function}
...
function checkEqual(v1::Vector{Particle}, v2::Vector{Particle}, offset=1) where Particle
  for i = offset:length(v1)
    if v1[i] != v2[i]
      return false
    end
  end
...
function _sample!(out::Particle, ws::Vector{Float64}, xs::Vector{Particle},
  M!::F, p::Int64, pScratch::ParticleScratch) where {Particle, F<:Function,
  ParticleScratch}
  ...
  M!(out, rng, p, xs[i], pScratch)
end

# Normalised probabilities from log weights ('log-sum-trick')
function _normalise_log_weights!(w, logw) 
  m = maximum(logw)
